...
class PlaylistSong extends Component {
	...
	handleClick() {
		...
		let times = document.getElementsByClassName('playlistSongTime');
	...
	componentDidMount() {
		...
		this.setState({
			...
			time: time + ":" + seconds,
			album: this.props.data.album.name
		})
	}
		...
		return (
			<div className="artistSong" onClick={this.handleClick}>
				...
				<div className="playlistContent">
					...
					<h4 className="albumSongArtist"> {this.state.artist + " • " + this.state.album} </h4>
				</div>
				<h4 id={this.state.time} className="playlistSongTime"> {this.state.time} </h4>
			</div>
		);
	}
}
